...
const dropFeedData = [{
  ...
  comments: [{
    ...
    text: "I've been waiting for this collab!",
    timestamp: '1h ago'
  }, {
    id: 2,
    username: 'MusicInsider',
    avatar: 'https://i.pravatar.cc/150?img=42',
    text: 'My sources confirm this is happening 💯',
    timestamp: '30m ago'
  }]
}, {
  id: 2,
...
const UpcomingReleaseListItem = ({
  ...
  return <div className="flex items-center bg-audra-dark rounded-lg p-3 mb-3 border border-white/10">
  ...
  return <div className="bg-audra-dark rounded-2xl p-4 mb-4 border border-white/10">
      ...
      <div className="bg-black/40 rounded-lg p-3 mb-3">
        ...
        <div className="h-2 bg-audra-gray rounded-full mb-3">
          <div className="h-full bg-gradient-teal-purple rounded-full" style={{
          width: `${dropProbability}%`
        }} />
        </div>
      ...
      {/* Comments section */}
      <div>
        <button onClick={toggleComments} className="flex items-center text-white/70 hover:text-white gap-1 text-xs mb-2">
          <MessageCircle size={16} />
          <span>{comments.length} comment{comments.length !== 1 ? 's' : ''}</span>
        </button>
        
        {isCommentsOpen && <div className="pt-2">
            ...
            <form onSubmit={handleAddComment} className="flex gap-2 items-center">
              ...
              <input type="text" placeholder="Add a comment..." value={newComment} onChange={e => setNewComment(e.target.value)} className="flex-1 bg-black/30 rounded-full px-3 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-audra-teal" />
              <Button type="submit" size="sm" className="h-7 px-3 text-xs bg-gradient-teal-purple hover:opacity-90">
                Post
              </Button>
            </form>
          </div>}
...
const Index = () => {
  const HeaderActions = () => <div className="flex items-center space-x-4">
      <Bell size={20} />
      <Settings size={20} />
    </div>;
  return <MobileLayout title="Drop Feed" rightAction={<HeaderActions />}>
      <div className="p-4">
        {/* Curated NMF section */}
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gradient-teal-purple mb-4">Curated NMF</h2>
          <div className="space-y-1">
            {upcomingReleases.map(release => <UpcomingReleaseListItem key={release.id} {...release} />)}
          </div>
        </div>
        ...
        <div>
          ...
          {dropFeedData.map(item => <DropFeedCard key={item.id} {...item} />)}
        </div>
      </div>
    </MobileLayout>;
};
